...
const DataGajiPegawai = () => {
    ...
    try {
      ...
      if (yearData.length > 0 && monthData.length > 0 && nameData.length > 0) {
        ...
        Swal.fire({
          ...
          text: "Désolé, les données que vous cherchez n'ont pas été trouvées",
          timer: 2000,
        });
      }
    } catch (error) {
    ...
    for (let page = startPage; page <= endPage; page++) {
      items.push(
        <button
          ...
          className={`border border-gray-2 px-4 py-2 font-semibold text-black dark:text-white ${currentPage === page
  ...
  return (
    <Layout>
      <Breadcrumb pageName="Données de Salaire de l'Utilisateur" />
        ...
        <div className="mt-4 flex flex-col items-center justify-between md:flex-row md:justify-between">
          <div className="flex items-center space-x-2">
            <span className="text-gray-5 dark:text-gray-4 py-4 text-sm">
              ...
              {Math.min(endIndex, dataGajiPegawai.length)} de{" "}
              {dataGajiPegawai.length} télégrammes
            </span>
          </div>
